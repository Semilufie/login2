...
public class CarDbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("create table car_table(_id integer primary key autoincrement, " +
                ...
                "product_img integer," +       // 注册类型   0---用户   1---管理员
                "product_count integer" +       // 注册类型   0---用户   1---管理员
                ")");
    ...
    public int addCar(String username, int product_id, int product_img, String product_title, int product_price) {
        // 判断商品是否添加到购物车 如果添加过只需要修改商品的数量即可
...
//            db.close();
    ...
    //根据用户名和商品id查询购物车有没有添加过
    @SuppressLint("Range")
    public CarInfo isAddCar(String username, int product_id) {
        //获取SQLiteDatabase实例
        SQLiteDatabase db = getReadableDatabase();
        CarInfo carInfo = null;
        String sql = "select _id,username,product_id,product_title,product_price,product_img,product_count from car_table where username=? and product_id=?";
        ...
        if (cursor.moveToNext()) {
            ...
            carInfo = new CarInfo(productId, car_id, name, product_img, product_title, product_price, product_count);
        }
        cursor.close();
//        db.close();
        return carInfo;
    ...
    public List<CarInfo> queryCarList(String username) {
        ...
        String sql = "select _id,username,product_id,product_title,product_price,product_img,product_count from car_table where username=?";
        ...
        while (cursor.moveToNext()) {
            ...
            int product_img = cursor.getInt(cursor.getColumnIndex("product_img"));
            int product_count = cursor.getInt(cursor.getColumnIndex("product_count"));
            list.add(new CarInfo(productId, car_id, name, product_img, product_title, product_price, product_count));
        }
        cursor.close();
//        db.close();
        return list;
